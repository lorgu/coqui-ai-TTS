import logging
from collections.abc import Callable
from pydoc import text
from typing import Union

from numpy import char

from TTS.tts.models.vits import CharactersConfig
from sympy import im

from TTS.tts.utils.text import cleaners
from TTS.tts.utils.text.characters import BaseCharacters, Graphemes, IPAPhonemes
from TTS.tts.utils.text.phonemizers import DEF_LANG_TO_PHONEMIZER, get_phonemizer_by_name
from TTS.tts.utils.text.phonemizers.multi_phonemizer import MultiPhonemizer
from TTS.utils.generic_utils import get_import_path, import_class
import ipdb
logger = logging.getLogger(__name__)


class TTSTokenizer:
    """🐸TTS tokenizer to convert input characters to token IDs and back.

    Token IDs for OOV chars are discarded but those are stored in `self.not_found_characters` for later.

    Args:
        use_phonemes (bool):
            Whether to use phonemes instead of characters. Defaults to False.

        characters (Characters):
            A Characters object to use for character-to-ID and ID-to-character mappings.

        text_cleaner (callable):
            A function to pre-process the text before tokenization and phonemization. Defaults to None.

        phonemizer (Phonemizer):
            A phonemizer object or a dict that maps language codes to phonemizer objects. Defaults to None.

    Example:

        >>> from TTS.tts.utils.text.tokenizer import TTSTokenizer
        >>> from TTS.tts.utils.text.tokenizer_phoneme_input import TTSTokenizer as TTSTokenizerPhoneme
        >>> from TTS.tts.utils.text.tokenizer_phoneme_input2 import TTSTokenizer as TTSTokenizerPhoneme2
        >>> from TTS.tts.utils.text.characters import BaseCharacters, Graphemes, IPAPhonemes
        >>> tokenizer = TTSTokenizer(use_phonemes=False, characters=Graphemes())
        >>> tokenizer_phoneme = TTSTokenizerPhoneme(use_phonemes=True, characters=IPAPhonemes())
        >>> tokenizer_phoneme2 = TTSTokenizerPhoneme2(use_phonemes=True, characters=IPAPhonemes())
        
        >>> text = "Hello world!"
        >>> ids = tokenizer.text_to_ids(text)
        >>> ids1 = tokenizer_phoneme.text_to_ids("h ə l oʊ w ɜr l d")
        >>> ids2 = tokenizer_phoneme2.text_to_ids("h ə l oʊ w ɜr l d")
        >>> text_hat = tokenizer.ids_to_text(ids)
        >>> text_hat1 = tokenizer_phoneme.ids_to_text(ids1)
        >>> text_hat2 = tokenizer_phoneme2.ids_to_text(ids2)
        >>> assert text == text_hat
    """
   
    def __init__(
        self,
        use_phonemes: bool = True,
        text_cleaner: Callable[[str], str] | None = None,
        # characters: # use the defined characters above
        characters: BaseCharacters | None = None,
        phonemizer: Union["Phonemizer", dict] | None = None,
        add_blank: bool = False,
        use_eos_bos: bool = False,
    ):
        self.text_cleaner = text_cleaner
        self.use_phonemes = use_phonemes
        self.add_blank = add_blank
        self.use_eos_bos = use_eos_bos
        self.characters = characters
        self.not_found_characters = []
        self.phonemizer = phonemizer
        print("Using direct phoneme input")
        print("use_phonemes:", self.use_phonemes)
        print("text_cleaner:", self.text_cleaner)
        print("characters:", self.characters)
        print("phonemizer:", self.phonemizer)
        print("add_blank:", self.add_blank)
        print("use_eos_bos:", self.use_eos_bos)

    @property
    def characters(self):
        return self._characters

    @characters.setter
    def characters(self, new_characters):
        self._characters = new_characters
        self.pad_id = self.characters.char_to_id(self.characters.pad) if self.characters.pad else None
        self.blank_id = self.characters.char_to_id(self.characters.blank) if self.characters.blank else None

    def encode(self, text: str) -> list[int]:
        """Encodes a string of text as a sequence of IDs."""
        token_ids = []
        for char in text:
            try:
                idx = self.characters.char_to_id(char)
                token_ids.append(idx)
            except KeyError:
                # discard but store not found characters
                if char not in self.not_found_characters:
                    self.not_found_characters.append(char)
                    logger.warning(text)
                    logger.warning("Character %s not found in the vocabulary. Discarding it.", repr(char))
        return token_ids

    def encode_phonemes(self, phonemes: list[str]) -> list[int]:
        token_ids = []
        for phoneme in phonemes:
            try:
                idx = self.characters.char_to_id(phoneme)
                token_ids.append(idx)
            except KeyError:
                if phoneme not in self.not_found_characters:
                    self.not_found_characters.append(phoneme)
                    logger.warning("Phoneme %s not found in the vocabulary. Discarding it.", repr(phoneme))
        # print("Phoneme token_ids (in encode_phonemes): ", token_ids)
        return token_ids

    def decode(self, token_ids: list[int]) -> str:
        """Decodes a sequence of IDs to a string of text."""
        text = ""
        for token_id in token_ids:
            text += self.characters.id_to_char(token_id)
        return text

    # def text_to_ids(self, text: str, language: str | None = None) -> list[int]:  # pylint: disable=unused-argument
    #     """Converts a string of text to a sequence of token IDs.

    #     Args:
    #         text(str):
    #             The text to convert to token IDs.

    #         language(str):
    #             The language code of the text. Defaults to None.

    #     TODO:
    #         - Add support for language-specific processing.

    #     1. Text normalizatin
    #     2. Phonemization (if use_phonemes is True)
    #     3. Add blank char between characters
    #     4. Add BOS and EOS characters
    #     5. Text to token IDs
    #     """
    #     # TODO: text cleaner should pick the right routine based on the language
    #     # in case i have phoneme input, you need to set the text cleaner to None to avoid cleaning
    #     logger.debug("Tokenizer input text: %s", text)
    #     if self.text_cleaner is not None:
    #         print("Using text_cleaner")
    #         text = self.text_cleaner(text)
    #         logger.debug("Cleaned text: %s", text)
    #     elif self.text_cleaner is None and self.use_phonemes:
    #         print("Phonemes: ", text)
        
    #     if self.use_phonemes and self.text_cleaner is not None:
    #         print("Using text_cleaner")
    #         text = self.phonemizer.phonemize(text, separator="", language=language)
    #         print(text)
    #         logger.debug("Phonemes: %s", text)
            
    #     if self.use_phonemes and self.text_cleaner is None:
    #         print("using encoder_phonemes")
    #         # text must be a list of phonemes, not a string with spaces.
    #         # i want text to be a string (separated by spaces)
    #         text = text.split(" ")
    #         text_list = [t for t in text if t != ""]  # removes empty strings
    #         text = self.encode_phonemes(text_list)     # pass the cleaned list of phonemes
    #     else:
    #         text = self.encode(text)
            
    #     if self.use_phonemes and self.text_cleaner is None:
    #         print("Text after encode_phonemes:", text)
    #     else:
    #         if self.add_blank:
    #             text = self.intersperse_blank_char(text, True)
    #         if self.use_eos_bos:
    #             text = self.pad_with_bos_eos(text)
    #     print("End of text2ids:", text)
    #     return text

    def text_to_ids(self, text: str, language: str | None = None):
        logger.debug("Tokenizer input text: %s", text)
        language = "de"
        # --- Case 1: Phoneme input provided directly ---
        if self.use_phonemes and self.text_cleaner is None:
            phonemes = [p for p in text.split(" ") if p]
            ids = self.encode_phonemes(phonemes)

        # --- Case 2: Raw text → phonemizer ---
        else:
            if self.text_cleaner:
                text = self.text_cleaner(text)

            if self.use_phonemes:
                phoneme_str = self.phonemizer.phonemize(text, separator=" ", language=language)
                phonemes = phoneme_str.split(" ")
                ids = self.encode_phonemes(phonemes)
            else:
                ids = self.encode(text)

        # --- Add blank / BOS / EOS ---
        if self.add_blank:
            ids = self.intersperse_blank_char(ids, True)

        if self.use_eos_bos:
            ids = self.pad_with_bos_eos(ids)

        # print("Phoneme token_ids (in text_to_ids): ", ids)
        return ids

    def ids_to_text(self, id_sequence: list[int]) -> str:
        """Converts a sequence of token IDs to a string of text."""
        return self.decode(id_sequence)

    def pad_with_bos_eos(self, char_sequence: list[str]):
        """Pads a sequence with the special BOS and EOS characters."""
        return [self.characters.bos_id] + list(char_sequence) + [self.characters.eos_id]

    def intersperse_blank_char(self, char_sequence: list[str], use_blank_char: bool = False):
        """Intersperses the blank character between characters in a sequence.

        Use the ```blank``` character if defined else use the ```pad``` character.
        """
        char_to_use = self.characters.blank_id if use_blank_char else self.characters.pad
        result = [char_to_use] * (len(char_sequence) * 2 + 1)
        result[1::2] = char_sequence
        return result

    def print_logs(self, level: int = 0):
        indent = "\t" * level
        logger.info("%s| add_blank: %s", indent, self.add_blank)
        logger.info("%s| use_eos_bos: %s", indent, self.use_eos_bos)
        logger.info("%s| use_phonemes: %s", indent, self.use_phonemes)
        if self.use_phonemes:
            logger.info("%s| phonemizer:", indent)
            if self.phonemizer is None:
                logger.info("%s| None (assuming pre-phonemized input)", indent)
            else:
                self.phonemizer.print_logs(level + 1)
        if len(self.not_found_characters) > 0:
            logger.info("%s| %d characters not found:", indent, len(self.not_found_characters))
            for char in self.not_found_characters:
                logger.info("%s| %s", indent, char)

    @staticmethod
    def init_from_config(config: "Coqpit", characters: BaseCharacters | None = None):
        """Init Tokenizer object from config

        Args:
            config (Coqpit): Coqpit model config.
            characters (BaseCharacters): Defines the model character set. If not set, use the default options based on
                the config values. Defaults to None.
        """
        # init cleaners
        text_cleaner = None
        if isinstance(config.text_cleaner, str | list):
            text_cleaner = getattr(cleaners, config.text_cleaner)
        # ipdb.set_trace()
        # init characters
        # if characters is None:
        #     # set characters based on defined characters class
        #     if config.characters and config.characters.characters_class:
        #         print(f"✅ Using custom character class: {config.characters.characters_class}")
        #         CharactersClass = import_class(config.characters.characters_class)
        #         if not issubclass(CharactersClass, BaseCharacters):
        #             msg = f"{config.characters.characters_class} is not a subclass of BaseCharacters."
        #             raise TypeError(msg)
        #         characters, new_config = CharactersClass.init_from_config(config)
        #     # set characters based on config
        #     else:
        #         if config.use_phonemes:
        #             # init phoneme set (regardless of text_cleaner)
        #             print("⚠️  Initializing default IPA phoneme characters (132 chars)")
        #             characters, new_config = IPAPhonemes().init_from_config(config)
        #         else:
        #             # init character set
        #             print("Initializing grapheme characters")
        #             characters, new_config = Graphemes().init_from_config(config)

        # else:
        #     characters, new_config = characters.init_from_config(config)
        # set characters_class to my phoneme class if None
        if config.characters.characters_class is None:
            print("✅ Setting characters_class to TTS.tts.models.vits.VitsPhonemeCharacters")
            config.characters.characters_class = "TTS.tts.models.vits.VitsPhonemeCharacters"
        CharactersClass = import_class(config.characters.characters_class)
        characters, new_config = CharactersClass.init_from_config(config)
        # set characters class
        new_config.characters.characters_class = get_import_path(characters)

        # init phonemizer
        phonemizer = None
        # Only initialize phonemizer if:
        # 1. use_phonemes is True AND
        # 2. phonemizer is explicitly set in config (not None)
        # If phonemizer is None, we assume input is already phonemized
        if config.use_phonemes and hasattr(config, 'phonemizer') and config.phonemizer is not None:
            if config.phonemizer == "multi_phonemizer":
                lang_to_phonemizer_name = {}
                for dataset in config.datasets:
                    if dataset.language != "":
                        lang_to_phonemizer_name[dataset.language] = dataset.phonemizer
                    else:
                        raise ValueError("Multi phonemizer requires language to be set for each dataset.")
                phonemizer = MultiPhonemizer(lang_to_phonemizer_name)
            else:
                phonemizer_kwargs = {"language": config.phoneme_language}
                phonemizer = get_phonemizer_by_name(config.phonemizer, **phonemizer_kwargs)
        elif config.use_phonemes and (not hasattr(config, 'phonemizer') or config.phonemizer is None):
            # If phonemizer is None but use_phonemes is True, assume pre-phonemized input
            print("⚠️  use_phonemes=True but phonemizer=None: Assuming input text is already phonemized")
            phonemizer = None
        elif config.use_phonemes:
            # Fallback: try to get default phonemizer for the language
            try:
                phonemizer_kwargs = {"language": config.phoneme_language}
                phonemizer = get_phonemizer_by_name(
                    DEF_LANG_TO_PHONEMIZER[config.phoneme_language], **phonemizer_kwargs
                )
                new_config.phonemizer = phonemizer.name()
                print(f"ℹ️  Using default phonemizer: {phonemizer.name()}")
            except KeyError as e:
                raise ValueError(
                    f"""No phonemizer found for language {config.phoneme_language}.
                    You may need to install a third party library for this language."""
                ) from e

        return (
            TTSTokenizer(
                config.use_phonemes, text_cleaner, characters, phonemizer, config.add_blank, config.enable_eos_bos_chars
            ),
            new_config,
        )